...
public class JoinLookupRequest extends LookupRequest implements Event {
    ...
    protected JoinLookupRequest(int protocol, NodeConnection connection, String channel, String lookupID, String myID) {
        super(protocol, connection, channel, lookupID, myID);
        routingTable = new ArrayList<>(4);
        for(int i=0; i < 4; i++)
            routingTable.add(new ArrayList<PeerNodeData>());
    }

    protected JoinLookupRequest(int protocol, NodeConnection connection, String channel, String lookupID, String[] route, String myID, List<List<PeerNodeData>> table) {
        super(protocol, connection, channel, lookupID, route, myID);
        routingTable = table;
    }
